...
persistent and change the configuration.
    ...
    --name mqtt jonasheinisch/rpi-mosquitto
...
The example uses a docker volume named `mosquitto_data` (see above).
    ...
    Restart=always
    ExecStart=/usr/bin/docker run -v /srv/mqtt/config:/mqtt/config -v /srv/mqtt/log:/mqtt/log -v mqtt-persistence:/mqtt/data/ -p ${EXT_IP}:1883:1883 -p ${EXT_IP}:8883:8883 -p 127.0.0.1:9001:9001 --name mqtt jonasheinisch/rpi-mosquitto
